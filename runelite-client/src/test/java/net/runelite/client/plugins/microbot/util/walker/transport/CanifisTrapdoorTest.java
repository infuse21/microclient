...
public class CanifisTrapdoorTest
{
	@Test
	public void dispatchRechecksQuestAndUsesDirectOpen()
	{
		Transport row = Transport.loadAllFromResources().values().stream().flatMap(Set::stream)
			.filter(t -> t.getObjectId() == 5055).findFirst().orElseThrow();
		...
		when(object.getId()).thenReturn(5055);
		when(object.getWorldLocation()).thenReturn(new WorldPoint(3495, 3466, 0));
			...
			mockStatic(net.runelite.client.plugins.microbot.util.player.Rs2Player.class))
		{
			...
			{
				player.when(() -> net.runelite.client.plugins.microbot.util.player.Rs2Player.getQuestState(
					Quest.IN_SEARCH_OF_THE_MYREQUE)).thenReturn(state);
				assertEquals(state == QuestState.FINISHED ? Rs2CatalogTransitionScene.DispatchResult.ISSUED
					: Rs2CatalogTransitionScene.DispatchResult.REJECTED, Rs2CatalogTransitionScene.dispatch(edge, "Open", 5055));
			}
	...
	@Test
	public void bothApproachesRequireTheQuestAndUndergroundLanding()
	{
		List<Transport> rows = Transport.loadAllFromResources().values().stream().flatMap(Set::stream)
			.filter(row -> row.getObjectId() == 5055).collect(java.util.stream.Collectors.toList());
		assertEquals(2, rows.size());
		for (Transport row : rows)
		{
			assertEquals(Map.of(Quest.IN_SEARCH_OF_THE_MYREQUE, QuestState.FINISHED), row.getQuests());
			...
			RouteInteraction pending = new RouteInteraction(1, 0, row.getOrigin(), row.getDestination(),
				...
				"Open", true, 5055, row.getOrigin(), row.getDestination());
			CatalogTransitionScene missingScene = mock(CatalogTransitionScene.class);
			CatalogTransitionRouteScanner scanner = new CatalogTransitionRouteScanner();
			assertNotEquals(RouteInteraction.Status.CLEARED,
				scanner.observePending(pending, row.getOrigin(), missingScene, 6).getStatus());
			assertEquals(RouteInteraction.Status.CLEARED,
				scanner.observePending(pending, new WorldPoint(3477, 9845, 0), missingScene, 6).getStatus());
			row.getQuests().clear();
