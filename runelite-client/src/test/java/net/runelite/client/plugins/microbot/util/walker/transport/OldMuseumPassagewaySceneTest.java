...
public class OldMuseumPassagewaySceneTest
{
	@Test
	public void allFourApproachesResolveTheirBridgeAdjustedCacheAnchor()
	{
		List<Transport> rows = Transport.loadAllFromResources().values().stream().flatMap(Set::stream)
			.filter(t -> t.getObjectId() == 31892).collect(java.util.stream.Collectors.toList());
	...
	private static void verifyDispatch(Transport row)
	{
		...
		when(object.getId()).thenReturn(31892);
		when(object.getWorldLocation()).thenReturn(new WorldPoint(row.getOrigin().getX() == 3014 ? 3013 : 3066, 9951, 0));
		...
		when(definition.getName()).thenReturn("Old passageway");
		when(definition.getActions()).thenReturn(new String[]{"Leave"});
		when(object.click("Leave")).thenReturn(true);
			...
			mockStatic(net.runelite.client.plugins.microbot.util.player.Rs2Player.class))
		{
			...
			assertEquals(Rs2CatalogTransitionScene.DispatchResult.ISSUED,
				Rs2CatalogTransitionScene.dispatch(edge, "Leave", 31892));
			when(object.getId()).thenReturn(47316);
			assertEquals(Rs2CatalogTransitionScene.DispatchResult.REJECTED,
				Rs2CatalogTransitionScene.dispatch(edge, "Leave", 31892));
			verify(object, times(1)).click("Leave");
		}
	}
